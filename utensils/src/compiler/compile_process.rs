...
pub fn exec(app_dir: &Path, src_file_path: &Path) {
    let process_name = src_file_path.file_stem().unwrap().to_str().unwrap();

    //let config_display = system::get_compile_config(&mut command, &current_dir, &src_file_path, &target_dir);
    println!("Compiling {}", process_name/*, config_display*/);
    ...
    system::rustc_compile_lib(app_dir, &dep_dirs, &src_file_path, worldsong_config::get_process_lib_type(), true);
}
