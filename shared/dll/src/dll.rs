...
impl DLL {
    ...
    pub fn open(filename: Option<&Path>) -> Result<DLL, String> {
        ...
        match maybe_library {
            Err(err) => Err(err),
            Ok(handle) => Ok(DLL { handle: handle })
        }
    }

    /// Prepends a path to this process's search path for dynamic libraries
    pub fn prepend_search_path(path: &Path) {
        let mut search_path = DLL::search_path();
        search_path.insert(0, path.to_path_buf());
        env::set_var(DLL::envvar(), &DLL::create_path(&search_path));
    }

    /// From a slice of paths, create a new vector which is suitable to be an
    /// environment variable for this platforms dll search path.
    pub fn create_path(path: &[PathBuf]) -> OsString {
        let mut newvar = OsString::new();
        for (i, path) in path.iter().enumerate() {
            if i > 0 { newvar.push(DLL::separator()); }
            newvar.push(path);
        }
        return newvar;
    }
